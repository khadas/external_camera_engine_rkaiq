...
XCamReturn HisteqSelectParam
(
    HisteqContext_t* pHisteqCtx,
    histeq_param_t* out,
    int iso)
{
    ...
    out->dyn.sw_hist_mapUserSet = interpolation_f32(
        paut->dyn[ilow].sw_hist_mapUserSet, paut->dyn[ihigh].sw_hist_mapUserSet, ratio);
    out->dyn.hw_histc_noiseCount_offset = interpolation_u8(
        paut->dyn[ilow].hw_histc_noiseCount_offset, paut->dyn[ihigh].hw_histc_noiseCount_offset, uratio);
    out->dyn.sw_histc_noiseCount_scale = interpolation_f32(
        paut->dyn[ilow].sw_histc_noiseCount_scale, paut->dyn[ihigh].sw_histc_noiseCount_scale, ratio);
    out->dyn.sw_histc_countWgt_minLimit = interpolation_f32(
        paut->dyn[ilow].sw_histc_countWgt_minLimit, paut->dyn[ihigh].sw_histc_countWgt_minLimit, ratio);
    ...
    for (i=0; i<17; i++) {
        out->dyn.sw_dehaze_outputMerge_alpha[i] = interpolation_f32(
            paut->dyn[ilow].sw_dehaze_outputMerge_alpha[i], paut->dyn[ihigh].sw_dehaze_outputMerge_alpha[i], ratio);
    }
#endif
    ...
    out->dyn.sw_hist_mapCount_scale = interpolation_f32(
        paut->dyn[ilow].sw_hist_mapCount_scale, paut->dyn[ihigh].sw_hist_mapCount_scale, ratio);
    out->dyn.hw_hist_paramTfilt_curWgt = interpolation_f32(
        paut->dyn[ilow].hw_hist_paramTfilt_curWgt, paut->dyn[ihigh].hw_hist_paramTfilt_curWgt, ratio);
    out->dyn.hw_hist_gainRef_sel = interpolation_f32(
        paut->dyn[ilow].hw_hist_gainRef_sel, paut->dyn[ihigh].hw_hist_gainRef_sel, ratio);
    out->dyn.sw_hist_globalMergePos_weight = interpolation_f32(
        paut->dyn[ilow].sw_hist_globalMergePos_weight, paut->dyn[ihigh].sw_hist_globalMergePos_weight, ratio);
    out->dyn.sw_hist_globalMergeNeg_weight = interpolation_f32(
        paut->dyn[ilow].sw_hist_globalMergeNeg_weight, paut->dyn[ihigh].sw_hist_globalMergeNeg_weight, ratio);
    out->dyn.sw_hist_mapMerge_alpha = interpolation_f32(
        paut->dyn[ilow].sw_hist_mapMerge_alpha, paut->dyn[ihigh].sw_hist_mapMerge_alpha, ratio);
    for (i=0; i<17; i++) {
        out->dyn.sw_hist_outputMerge_pos_alpha[i] = interpolation_f32(
            paut->dyn[ilow].sw_hist_outputMerge_pos_alpha[i], paut->dyn[ihigh].sw_hist_outputMerge_pos_alpha[i], ratio);
        out->dyn.sw_hist_outputMerge_neg_alpha[i] = interpolation_f32(
            paut->dyn[ilow].sw_hist_outputMerge_neg_alpha[i], paut->dyn[ihigh].sw_hist_outputMerge_neg_alpha[i], ratio);
    }
#endif
...
)
{
    ...
    ctx->isReCal_ = true;
    ctx->histeq_attrib =
        (histeq_api_attrib_t*)(CALIBDBV2_GET_MODULE_PTR(pCalibDbV2, histEQ));
