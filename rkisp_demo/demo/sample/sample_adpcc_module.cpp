...
static void sample_dpc_tuningtool_test(const rk_aiq_sys_ctx_t* ctx)
{
    ...
    std::string json_dpc_status_str = " \n\
        [{ \n\
            \"op\":\"get\", \n\
    ...
    rkaiq_uapi_unified_ctl(const_cast<rk_aiq_sys_ctx_t*>(ctx),
                           const_cast<char*>(json_dpc_status_str.c_str()), &ret_str, RKAIQUAPI_OPMODE_GET);
    ...
    ret_str = NULL;
    rkaiq_uapi_unified_ctl(const_cast<rk_aiq_sys_ctx_t*>(ctx),
                           const_cast<char*>(json_dpc_str.c_str()), &ret_str, RKAIQUAPI_OPMODE_SET);
...
void get_auto_attr(dpc_api_attrib_t* attr) {
    dpc_param_auto_t* stAuto = &attr->stAuto;
    for (int i = 0;i < 13;i++) {
    }
}

void get_manual_attr(dpc_api_attrib_t* attr) {
